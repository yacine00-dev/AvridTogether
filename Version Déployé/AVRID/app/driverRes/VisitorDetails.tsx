...
export default function VisitorDetailsScreen() {
  ...
  const api = axios.create({
    baseURL: 'https://testat-hsrz.onrender.com/api/',
    timeout: 10000,
  });
  ...
  const fetchUserProfile = async (username: string): Promise<string | null> => {
    try {
      ...
      if (response.data?.user?.user_pic) {
        return `https://testat-hsrz.onrender.com${response.data.user.user_pic}`;
      }
      return null;
    } catch (error) {
        ...
        <View style={styles.profileSection}>
          <Image 
            source={
              visitor?.user_pic 
                ? { 
                    uri: `https://testat-hsrz.onrender.com${visitor.user_pic}`, 
                    cache: 'reload',
                    headers: {
                      Pragma: 'no-cache'
                    }
